    ...
    <section className="py-14 bg-white">
      <div className="max-w-screen-xl mx-auto px-4">
        <div className="text-center mb-10">
          <h2 className="section-title mb-2">Ce que disent nos clients</h2>
          <div className="flex items-center justify-center gap-3 mt-3">
            ...
            <span className="font-black text-2xl">{averageRating}/5</span>
            <span className="text-gray-500 text-sm">basé sur {totalReviews} avis vérifiés</span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {reviews.map(review => (
            <div key={review.id} className="bg-gray-50 p-5 relative hover:shadow-md transition-shadow">
              ...
              <div className="flex mb-3">
                {[1, 2, 3, 4, 5].map(s => (
                  <Star
                    key={s}
                    size={14}
                    className={s <= review.rating ? 'text-[#fff500] fill-[#fff500]' : 'text-gray-300 fill-gray-300'}
                  />
                ))}
              </div>
              <p className="text-sm text-gray-700 leading-relaxed mb-4">"{review.comment}"</p>
              <div className="border-t border-gray-200 pt-3">
                <p className="font-bold text-sm text-black">{review.author}</p>
                <p className="text-xs text-gray-500">{review.location} — {review.date}</p>
                <p className="text-xs text-gray-400 mt-1 italic">{review.product}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm text-gray-500">
            Avis vérifiés par nos clients. <span className="font-semibold text-black">+{totalReviews} avis</span> sur l'ensemble de nos produits.
          </p>
        </div>
      </div>
    </section>
